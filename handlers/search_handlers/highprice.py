...
def start_highprice(call: CallbackQuery) -> None:
    ...
    bot.send_message(call.message.chat.id, 'Отлично! Вы выбрали поиск самых дорогих отелей. Выберите город для поиска.')


@bot.message_handler(commands=['highprice'])
def start_highprice(message: Message) -> None:
    """
    Начало работы команды поиска самых дорогих отелей
    :return: None
    """
    logger.info(f'user_id {message.from_user.id}')
    bot.set_state(message.from_user.id, HighPriceStates.cities, message.chat.id)
    bot.send_message(message.chat.id, 'Отлично! Вы выбрали поиск самых дорогих отелей. Выберите город для поиска.')
...
def get_cities_request(message: Message) -> None:
    ...
    with bot.retrieve_data(message.from_user.id, message.chat.id) as data:
        ...
        logger.info(f'user_id {message.from_user.id}')
        keyboard = get_destination_id(message.text, data['locale'], data['currency'], state='High_state')
...
def user_is_ready(message: Message, user_id=None, chat_id=None) -> None:
    ...
    with bot.retrieve_data(user_id, chat_id) as data:
        ex_str = get_properties_list(data['destination_id'], data["start_day"], data["end_day"], data['SortOrder'],
                                     data['locale'],
                                     data['currency'], data['count_hotels'], user_id, command='highprice',
